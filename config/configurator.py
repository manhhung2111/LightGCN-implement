...
def parse_configure():
    ...
    if not os.path.exists('./config/{}.yml'.format(model_name)):
        raise Exception("Please create the yaml file for the model first.")

    with open('./config/{}.yml'.format(model_name), encoding='utf-8') as f:
        config_data = f.read()
